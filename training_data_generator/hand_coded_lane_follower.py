...
line_color=(0, 255, 0)
line_width=10
max_angle_deviation_two_lines=0.6
max_angle_deviation_one_lane=0.6
_SHOW_IMAGE = True
class HandCodedLaneFollower(object):
    ...
    def follow_lane(self, frame):
        ...
        upper_black = np.array([179, 120, 100])
        ...
        #polygon = np.array([[
        #        (width*1/20, height*6/10),
        #        (width*19/20,height*6/10),
        #        (width*19/20,height*1),
        #        (width*1/20,height*1),
        #    ]], np.int32)
        polygon = np.array([[
        (0, height * 0.45),
        ...
        if line_segments is None:
            ...
            for line_segment in line_segments:
                for x1, y1, x2, y2 in line_segment:
                    ...
                    slope=fit[0] #(y2-y1)/(x2-x1)
                    intercept =fit[1] #y1-slope*x1
                    if slope < 0:
    ...
    #$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$
        ...
        new_steering_angle = angle_to_mid_deg + 90  # this is the steering angle needed by picar front wheel
        print("------------------------------")
        ...
        steering_angle_radian = self.curr_steering_angle / 180.0 * math.pi
        x1 = int(width / 2)
        y1 = height
        ...
        return heading_image#final_frame
...
def detect_lane(frame):
        ...
        for line_segment in line_segments:
            for x1, y1, x2, y2 in line_segment:
                if x1 == x2:
                    ...
                    continue
                #fit = np.polyfit((x1, x2), (y1, y2), 1)
                slope = (y2-y1)/(x2-x1)#fit[0]
                intercept =y1-slope*x1 #fit[1]
                if slope < 0:
                    if x1 < left_region_boundary and x2 < left_region_boundary:
                        left_fit.append((slope, intercept))
                else:
    ...
    if lane_lines is not None:
        for line in lane_lines:
            for x1, y1, x2, y2 in line:
                cv2.line(line_image, (x1, y1), (x2, y2), line_color, line_width)
    lane_lines_image = cv2.addWeighted(frame, 0.8, line_image, 1, 1)
    #############################################################
    #show_image("lane lines", lane_lines_image)
...
def detect_line_segments(cropped_edges):
    ...
    line_segments = cv2.HoughLinesP(cropped_edges, rho, angle, min_threshold, np.array([]), minLineLength=8,
     ...
     #3   for line_segment in line_segments:
